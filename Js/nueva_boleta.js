...
$(document).ready(function() {
    ...
    $('#id_articulo').select2({
        ...
        allowClear: true,
        ajax: {
            // Esta es la URL que debe apuntar al controlador de Boletas
            url: '../controllers/BoletaController.php?op=listar_articulos_para_venta',
            dataType: 'json',
            delay: 250,
            ...
            processResults: function(data) {
                return {
                    results: $.map(data, function(item) {
                        return {
                            id: item.id,
                            text: item.nombre + ' (Stock: ' + item.stock + ')',
                            nombre: item.nombre,
    ...
    function renderizarCarrito() {
        ...
        if (lista_articulos_carrito.length === 0) {
            ...
            $.each(lista_articulos_carrito, function(index, item) {
                ...
                html += '</tr>';
                total_boleta += item.subtotal;
            });
        }
        $('#carrito_articulos_body').html(html);
        $('#total_boleta_display').text(total_boleta.toFixed(2));
    ...
    // Manejar el envío del formulario de la boleta
    $('#form_nueva_boleta').on('submit', function(e) {
        // Antes de enviar, asegurarse de que haya artículos en el carrito
        if (lista_articulos_carrito.length === 0) {
            e.preventDefault(); // Evitar el envío si el carrito está vacío
            Swal.fire('Atención', 'No puedes generar una boleta sin artículos en el carrito.', 'warning');
            return;
        }
